...
| 状态 | **提议**（待评审） |
| 决策日期 | 2026-09-17 |
| 决策载体 | 拟：《设计文档》v1.5 → **v1.6**；《实施计划与验收标准》v1.11 → **v1.12**（评审通过后执行，见附录 C/D 的合入文本） |
...
| 命令层测试（含 `startup_get` 快照、迁移、续跑、选择器） | `crates/aether-tauri/tests/m1_06_{startup_ipc,picker,migration}.rs` |
