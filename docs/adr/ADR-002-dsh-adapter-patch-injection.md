...
- 帧级丢失率 = Σ 缺口帧数 ÷ Σ 应有帧数：自研插件为每个 attempt 的带外帧（`start` / `chunk` / `end`）携带从 0 起的递增 `seq`；适配器按 `attemptId + seq` 归并（允许乱序，按 `seq` 重排），缺口帧数 = `max(seq) + 1 − 已收帧数`；
- 平均长度偏差 = 20 次 `|delta 总长度 − ACP final 长度| ÷ ACP final 长度` 的算术平均；
- 单次触发 §3.6 回退（未收到 `end` 或长度偏差 >5%）的 run，按 final 重建后仍计入上述两项统计。
...
| v0.5 | 2026-09-15 | 评审修订 §5.3：回退指标改为「帧级丢失率 >1% 或平均长度偏差 >5%」（§3.6 单次 5% 长度偏差阈值保留）；补指标口径（`seq` 缺口语义、聚合方式）；实施计划 v1.7 同步 M2-11 DoD7 | pymu |
